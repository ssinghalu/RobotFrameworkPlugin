...
public class RobotSuiteResult extends RobotTestObject {
	...
	public List<Ticket> getAllTickets() {
		...
		for(RobotCaseResult roboCase : getAllCases()){
			tickets.addAll(roboCase.getAllTickets());
		}
		return tickets;
	}
	...
	public int getAllFailedTicketCases() {
		int failedCases = 0;
		for(RobotCaseResult caseResult : getCaseResults()){
			if(!caseResult.isPassed()) failedCases = failedCases + caseResult.getFailTicketCount();
		}
		for(RobotSuiteResult suite : getChildSuites()){
			failedCases = failedCases + suite.getAllFailedTicketCases();
		}
		return failedCases;
	}
	...
	public int getAllPassedTicketCases() {
		int passedCases = 0;
		for(RobotCaseResult caseResult : getCaseResults()){
			if(caseResult.isPassed()) passedCases = passedCases + caseResult.getPassTicketCount();
		}
		for(RobotSuiteResult suite : getChildSuites()){
			passedCases = passedCases + suite.getAllPassedTicketCases();
		}
		return passedCases;
	}
}
