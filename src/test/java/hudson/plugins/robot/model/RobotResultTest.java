...
public class RobotResultTest extends TestCase {
	...
	public void testShouldExtractTicketNumbers() throws Exception {
		RobotParser.RobotParserCallable remoteOperation = new RobotParser.RobotParserCallable("tickets.xml", null, null);
		RobotResult res = remoteOperation.invoke(new File(RobotSuiteResultTest.class.getResource("tickets.xml").toURI()).getParentFile(), null);
		List<Ticket> tickets = res.getAllTickets();
		assertEquals(8, tickets.size());
		assertEquals(8, res.getTicketsCount());
	}
	
	public void testFailTicketcount() throws Exception {
		RobotParser.RobotParserCallable remoteOperation = new RobotParser.RobotParserCallable("test.xml", null, null);
		RobotResult res = remoteOperation.invoke(new File(RobotSuiteResultTest.class.getResource("test.xml").toURI()).getParentFile(), null);
		//List<Ticket> tickets = res.getAllTickets();
		//assertEquals(8, tickets.size());
		//assertEquals(8, res.getTicketsCount());
		int  failers = res.getAllFailedTicketCases();
		assertEquals(12, failers);
	}
	
	public void testPassTicketcount() throws Exception {
		RobotParser.RobotParserCallable remoteOperation = new RobotParser.RobotParserCallable("test.xml", null, null);
		RobotResult res = remoteOperation.invoke(new File(RobotSuiteResultTest.class.getResource("test.xml").toURI()).getParentFile(), null);
		//List<Ticket> tickets = res.getAllTickets();
		//assertEquals(8, tickets.size());
		//assertEquals(8, res.getTicketsCount());
		int  failers = res.getAllPassedTicketCases();
		assertEquals(12, failers);
	}
